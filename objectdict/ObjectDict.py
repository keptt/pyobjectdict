...
class ObjectDict():
    ...
    def __init__(self, dictionary: dict=None, type_=None):
        ...
        if type_: # is needed to be able to form() obj as an ordered dict
            if type_ not in [dict, OrderedDict]:
        ...
        elif type(dictionary) is ObjectDict:
            pass
    ...
    def __add__(self, other):
        if not isinstance(other, ObjectDict):
    ...
    def add_each_value(self, other): #!!!!
        ...
        return ObjectDict(self._dict_type({key: (value + other[key] if other.get(key) else value) for key, value in self.items()}))
    ...
    def __sub__(self, other):
        ...
        return ObjectDict(self._dict_type({key: value for key, value in self.items() if key not in other}))
    ...
    def sub_by_value(self, other):
        ...
        return ObjectDict(self._dict_type({key: value for key, value in self.items() if key not in other or value != other[key]}))
